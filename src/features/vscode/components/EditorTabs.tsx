...
export const EditorTabs = ({ tabs, activeId, onSelect, onClose }: EditorTabsProps) => {
  return (
    <div className="flex h-9 overflow-x-auto bg-[#1B1B1C]">
      {tabs.map((tab) => {
        ...
        return (
          <div
            key={tab.id}
            className={[
              'group flex min-w-[150px] items-center justify-between px-3 text-[11px] font-mono cursor-pointer',
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className={isActive ? 'text-on-surface' : 'text-on-surface-variant'}>{tab.name}</span>
            </div>
            <button
